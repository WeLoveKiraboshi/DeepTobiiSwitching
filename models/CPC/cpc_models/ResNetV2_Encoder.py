...
class PreActResNet_Encoder(nn.Module):
    def __init__(self, cfg, use_classifier, block, num_blocks, num_channels, eval=False):
        ...
        # If there isn't normalisation then the conv layers need biasing
        bias = True if (cfg.norm == "none") else False
        ...
        if self.dataset == "stl10":
            ...
            self.norm1 = norm2d(self.in_planes, cfg.norm)
        ...
        for i in range(len(num_blocks)):
            layers_array.append(self._make_layer(cfg, block, num_channels[i], num_blocks[i], stride=stride))
            if stride == 1:
                stride = 2
        self.layers = nn.Sequential(*layers_array)

        self.avgpool = nn.AdaptiveAvgPool2d(1)
        self.classifier = nn.Linear(num_channels[-1]*block.expansion, cfg.num_classes)

    def _make_layer(self, cfg, block, planes, num_blocks, stride):
        strides = [stride] + [1]*(num_blocks-1)
        layers = []
        for stride in strides:
            layers.append(block(cfg, self.in_planes, planes, stride))
...
def PreActResNet18_Encoder(cfg, use_classifier, eval=False):
    return PreActResNet_Encoder(cfg, use_classifier, PreActBlock, [2, 2, 2, 2], [64, 128, 256, 512], eval=eval)
...
def PreActResNetN_Encoder(encoder='resnet18', use_classifier=False, cfg=None, eval=False):
    if encoder == "resnet18":
        return PreActResNet18_Encoder(cfg, use_classifier,eval=eval)
