...
class MoCo(nn.Module):
    ...
    def _dequeue_and_enqueue(self, keys):
        ...
        # print('self que size = ', self.queue.shape)
        # print('idx {} - {}'.format(ptr, ptr + batch_size))
    ...
    def _batch_shuffle_ddp(self, x):
        ...
        *** Only support DistributedDataParallel (DDP) model. ***
        ...
        return x_gather, idx_unshuffle #x_gather[idx_this], idx_unshuffle
    ...
    def _batch_unshuffle_ddp(self, x, idx_unshuffle):
        ...
        *** Only support DistributedDataParallel (DDP) model. ***
        ...
        return x_gather #x_gather[idx_this]
...
def concat_all_gather(tensor):
    ...
    *** Warning ***: torch.distributed.all_gather has no gradient.
    """
    #tensors_gather = torch.ones_like(tensor) #[torch.ones_like(tensor) for _ in range(torch.distributed.get_world_size())]
    #torch.distributed.all_gather(tensors_gather, tensor, async_op=False)

    output = torch.cat(tensor, dim=0) # torch.cat(tensors_gather, dim=0)
    return output
