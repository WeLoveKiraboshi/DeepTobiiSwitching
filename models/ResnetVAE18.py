...
class BasicBlockDec(nn.Module):
    def __init__(self, in_planes, stride=1):
        ...
        #print('in planes : {}, planes : {}, stride : {}'.format(in_planes, planes, stride))
    def forward(self, x):
...
class ResNet18_Encoder(nn.Module):
    def __init__(self, fc_hidden1=1024, drop_p=0.3, CNN_embed_dim=256, pre_train=True):
        ...
        # encoding components
        resnet = models.resnet18(pretrained=pre_train)
...
class ResNet18_VAE(nn.Module):
    def __init__(self, z_dim=64, h_dim=1024, drop_p = 0.3, pre_train=True):
        super().__init__()
        self.encoder = ResNet18_Encoder(fc_hidden1=h_dim, drop_p=drop_p, CNN_embed_dim=z_dim, pre_train=pre_train)
        self.decoder = ResNet18_Decoder(fc_hidden1=h_dim, CNN_embed_dim=z_dim)
