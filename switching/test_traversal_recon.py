    ...
    with torch.no_grad():
        ...
        for i, sample_batched in tqdm(enumerate(dataset)):
            ...
            if cfg.network == 'resnet18' or cfg.network == 'resnet50':
                ...
                output_c, mu_c, logvar_c, z_c = model.forward(image_cams)
                #z_t = z_t.squeeze(0)
                z = torch.mean(z_t, 0).to('cpu').detach().numpy().copy() #.unsqueeze(0)
                break
        #z = np.mean(np.array(tensor_list), 0)
        #print(z.shape)
        ...
        for k in range(0, vertical_samples):
            z_0 = z
            list_of_z = []
            list_of_z.append(z_0)
            dim_observed = k
            z_temp = z_0 #np.copy(z_0)
            for i in range(0, horizontal_samples):
                z_temp[dim_observed+13] += 1.0
            ...
            for i in range(0, len(list_of_z)):
                Tensor_np = np.repeat(list_of_z[i][np.newaxis, :], cfg.bs, axis=0)
                Tensor = torch.from_numpy(Tensor_np).cuda()
                x_gen = model.generate(Tensor).permute(0, 2, 3, 1)
                x_gen = x_gen[:, :, :, [2, 1, 0]]
                # figidx = k * vertical_samples + i + 1
                # axs = fig.add_subplot(vertical_samples, horizontal_samples, figidx)
                # axs.imshow(x_gen[0].to('cpu').detach().numpy().copy())
                # axs.axes.get_xaxis().set_visible(False)
                # axs.axes.get_yaxis().set_visible(False)
                axs[j].imshow(x_gen[0].to('cpu').detach().numpy().copy())
...
def test():
    ...
    generate_images(dataloader_test)
