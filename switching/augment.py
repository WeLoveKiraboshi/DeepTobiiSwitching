...
class CenterCrop(object):
    ...
    def __init__(self, size, obtain_original_size = False, original_size=224):
        if isinstance(size, numbers.Number):
            self.size = (int(size), int(size))
            self.obtain_original_size = obtain_original_size
    ...
    def __call__(self, img):
        ...
        """
        if self.obtain_original_size:
            return F.resize(T.RandomResizedCrop(self.size, scale=(1.0, 1.0), ratio=(1.0, 1.0))(img), self.original_size, Image.BILINEAR)
        else:
            return T.RandomResizedCrop(self.size, scale=(1.0, 1.0), ratio=(1.0, 1.0))(img)
...
class ImageShrink(object):
    ...
    def __call__(self, img):
        """
        Args:
            img (PIL Image): Image to be shrinked.
        Returns:
            PIL Image: Shrinked image.
        """
        size_ = (int(img.size[0]*self.size), int(img.size[1]*self.size))
        resized = F.resize(img, size_, Image.BICUBIC)
        print(img.mode)
        dst_im = Image.new(img.mode, (img.size[0], img.size[1]), "black")
        col_margin = int((img.size[0] - resized.size[0])/2)
        row_margin = int((img.size[1] - resized.size[1]) / 2)
        print((col_margin, row_margin, img.size[0]-col_margin, img.size[1]-row_margin), resized.size)
        if img.size[0]-col_margin*2 == resized.size[0]:
            dst_im.paste(resized, (col_margin, row_margin, img.size[0]-col_margin, img.size[1]-row_margin))
        else:
            dst_im.paste(resized, (col_margin, row_margin, img.size[0] - col_margin-1, img.size[1] - row_margin-1))
        return dst_im
...
class ColorGamma(object):
    ...
    def __repr__(self):
        return self.__class__.__name__ + '(gamma transform gamma={0}, gain={1})'.format(self.gamma, self.gain)
...
class ColorJitter(object):
    ...
    def get_params(brightness, contrast, saturation, hue):
        ...
        if brightness is not None:
            ...
            #transforms.append(Lambda(lambda img: F.adjust_gamma(img, 2)))
...
class RandomAffineCV2(object):
    ...
    def affineTransformCV2(img, alpha, mask=None):
        ...
        img = cv2.warpAffine(img, M, imgsize[::-1],
                               flags=cv2.INTER_NEAREST, borderMode=None) #"""cv2.BORDER_REFLECT_101"""
        if mask is not None:
...
class mask_circle_solid(object):
    def __init__(self, background_color, blur_radius, offset=2):
        self.background_color = background_color
        self.blur_radius = blur_radius
        self.offset = offset

    def __call__(self, pil_img):
        background = Image.new(pil_img.mode, pil_img.size, self.background_color)
        mask = Image.new("L", pil_img.size, 0)
        draw = ImageDraw.Draw(mask)
        draw.ellipse((self.offset, self.offset, pil_img.size[0] - self.offset, pil_img.size[1] - self.offset), fill=255)
        mask = mask.filter(ImageFilter.GaussianBlur(self.blur_radius))
        return Image.composite(pil_img, background, mask)

class RandomCircleMasking(object):
    def __init__(self, background_color, blur_radius, offset=10, imsize=224):
        self.background_color = background_color
        self.blur_radius = blur_radius
        self.offset = offset
        self.imsize = 224
    ...
    def __call__(self, pil_img):
        ...
        draw = ImageDraw.Draw(mask)
        if np.random.ranf() < 0.2:  # 2 circles for 1/5 images
            n_trials = 2
        else:
            n_trials = 1 #np.random.randint(1, 2)
        for i in range(n_trials):
            diameter = np.random.randint(100, 200)
            cent_x = np.random.randint(self.offset, pil_img.size[0]-self.offset*1.8)
            cent_y = np.random.randint(self.offset, pil_img.size[1]-self.offset*1.8)
            draw.ellipse((cent_x, cent_y, cent_x + diameter, cent_y + diameter), fill=0)
        mask = mask.filter(ImageFilter.GaussianBlur(self.blur_radius))
        return Image.composite(pil_img, background, mask)
...
class RandomHomography(object):
    def __init__(self):
        #this is random homography code
        #print('Random Homography Addapt')
        """
        H = sample_homography_np(np.array([width, height]), shift=0, perspective=True, scaling=True, rotation=True,
                                 translation=False,
                                 n_scales=5, n_angles=25, scaling_amplitude=0.10, perspective_amplitude_x=0.1,
    ...
    def __call__(self, img):
        ...
        H = sample_homography_np(np.array([width, height]), shift=0, perspective=True, scaling=True, rotation=True,
                                 translation=False,
                                 n_scales=5, n_angles=25, scaling_amplitude=0.30, perspective_amplitude_x=0.3,
                                 perspective_amplitude_y=0.3, patch_ratio=0.5, max_angle=pi,
                                 allow_artifacts=False, translation_overflow=0.)
        ...
        destination_corner_list = np.array([[0, 0, 1], [0, height, 1], [width, height, 1], [width, 0, 1]])
        ...
        destination_corner_list = np.array(destination_corner_list).transpose()
        transformed_destination_corner_list = np.dot(H, destination_corner_list).transpose()
        tp = transformed_destination_corner_list / transformed_destination_corner_list[:, 2].reshape(4, 1)
        tp = tp[:, 0:2]
        # calc cross point
        a1 = (tp[2, 1] - tp[0, 1]) / (tp[2, 0] - tp[0, 0])
        a2 = (tp[3, 1] - tp[1, 1]) / (tp[3, 0] - tp[1, 0])
        b1 = tp[2, 1] - (tp[2, 1] - tp[0, 1]) * tp[2, 0] / (tp[2, 0] - tp[0, 0])
        b2 = tp[3, 1] - (tp[3, 1] - tp[1, 1]) * tp[3, 0] / (tp[3, 0] - tp[1, 0])
        center_x = int((b2 - b1) / (a1 - a2))
        center_y = int(a1 * center_x + b1)
        center_pos = np.array([center_x, center_y]).reshape(1, 2)
        diff = tp - center_pos
        L2 = np.sqrt(pow(diff[:, 0], 2) + pow(diff[:, 1], 2))
        L2max = int(np.max(L2))
        ...
        transformed_img = transformed_img.crop((center_x - L2max, center_y - L2max, center_x + L2max, center_y + L2max))
        F.resize(img, (224, 224), Image.BILINEAR)

        transformed_img.paste(img, (0, 0))
...
if __name__ == '__main__':
    ...
    cam1 = Image.open('cam1.png').resize((224, 224))
    homography = RandomHomography(torch.Tensor([224,224]))
    homography(cam1)